...
interface EditRecurringModalProps {
  ...
  onClose: () => void;
  onSave: (recurringId: string, data: Record<string, any>) => Promise<void>;
  currencyCode: string;
...
export const EditRecurringModal: React.FC<EditRecurringModalProps> = ({
  ...
  const [dayOfMonth, setDayOfMonth] = useState<number>(rule.dayOfMonth);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSave(rule.recurringId, {
        ...
        dayOfMonth,
        startDate,
        active: rule.active, // preserve current pause/resume state
      });
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-md max-h-full flex flex-col glass-card rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden min-h-0">
          ...
          <div className="p-5 border-t border-white/[0.06] shrink-0 bg-slate-950/20">
            <button
              type="submit"
